                        ...
                        int H, int W, int C_in, int C_out) 
{
    ...
    // Zero output buffer before GEMM to prevent NaN from uninitialized memory
...
int main() {
    ...
    // Use small random init to avoid identical weights that can amplify NaN
    auto init_rand = [](vector<float>& w, float scale) {
        w.resize(w.size());
        for (size_t i = 0; i < w.size(); ++i) {
            w[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * scale;
        }
    };
    ...
    srand(42);
    const float init_scale = 0.01f; // consistent with phase3 reference implementation
    for (size_t i = 0; i < w_conv1.size(); ++i) w_conv1[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * init_scale;
    for (size_t i = 0; i < w_conv2.size(); ++i) w_conv2[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * init_scale;
    for (size_t i = 0; i < w_dec1.size(); ++i) w_dec1[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * init_scale;
    for (size_t i = 0; i < w_dec2.size(); ++i) w_dec2[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * init_scale;
    for (size_t i = 0; i < w_final.size(); ++i) w_final[i] = (rand() / float(RAND_MAX) * 2.0f - 1.0f) * init_scale;
